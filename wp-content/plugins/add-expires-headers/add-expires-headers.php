...
if ( function_exists( 'dd_aeh' ) ) {
    ...
    if ( !class_exists( 'AEH_Pro' ) ) {
        class AEH_Pro
        {
            ...
            public function dd_aeh_uninstall_cleanup()
            {
            ...
            public function aeh_compat_checker()
            {
                ...
                if ( !in_array( 'mod_expires', apache_get_modules() ) ) {
                    $error = 'Add Expires Headers requires the apache mod_expirees module to be installed on the server.';
                }
                # wp version requirements
                if ( $nwpv < '4.5' ) {
                    $error = 'Add Expires Headers requires WP 4.5 or higher. You’re still on ' . $wp_version;
                }
