...
;(function($, window, document, undefined) {
    ...
    $.fn.CSFRAMEWORK_TAB_NAVIGATION = function() {
        return this.each(function() {
            ...
            $expand.on('click', function(e) {
                ...
                $(this).find('.fas').toggleClass('fa-eye-slash').toggleClass('fa-eye');
            });
        });
    };
    // ======================================================
    ...
    $.fn.CSFRAMEWORK_GROUP = function() {
        return this.each(function() {
            ...
            if (accordion_group.length) {
                ...
                // set title direct
                field_groups.find('.cs-group').each(function() {
                    var $_this = this;
                    $($_this).find('input').on('keyup', function(event) {
                        var rex = /(<([^>]+)>)/ig;
                        this.value = this.value.replace(rex , '');
                        $($_this).find('.cs-group-title').text(this.value);
                    });
                });
    ...
    $.fn.CSFRAMEWORK_SAVE = function() {
        return this.each(function() {
            ...
            $this.on('click', function(e) {
                ...
                $('.cs-fieldset input[type=text]').each(function() {
                    var $sanitize_input_field = $(this).val();
                    var rex = /(<([^>]+)>)/ig;
                    $sanitize_input_field = $(this).val().replace(rex , '');
                    $(this).val($sanitize_input_field);
                });
                ...
                if ($ajax.length) {
                    ...
                    $.post('options.php', serializedOptions).error(function() {
                        ...
                        $ajax.hide().fadeIn().delay(500).fadeOut();
                    });
                    e.preventDefault();
                } else {
    ...
    $.CSFRAMEWORK.SHORTCODE_MANAGER = function() {
        ...
        base.init = function() {
            $cs_body.on('click', '.cs-shortcode', function(e) {
                ...
                // var $d = $target, // this for remember last shortcode select
                var $d = $target.clone().appendTo($cs_body), // this clear select choise
                    $insert = $d.find('.cs-dialog-insert'),
                    $shortcodeload = $d.find('.cs-dialog-load'),
                    $selector = $d.find('.cs-dialog-select'),
                    ...
                    open: function() {
                        ...
                        $(window).on('resize', function() {
                            var height = $(window).height(),
                                ...
                                set_height = Math.floor(height - 125);
                            $d.dialog('option', 'height', set_height).parent().css('max-height', set_height);
                            $d.css('overflow', 'auto');
                ...
                $selector.on('change', function() {
                    ...
                    if (shortcode_name.length) {
                        $.ajax({
                            ...
                            url: ajaxurl,
                            data: {
                                action: targetSelector, // important unique id for ajax
                                shortcode: shortcode_name
                            },
                            success: function(content) {
                                $shortcodeload.html(content);
                                $insert.parent().removeClass('hidden');
                                shortcode_clone = $('.cs-shortcode-clone', $d).clone();
                ...
                $insert.on('click', function(e) {
                    ...
                    if (shortcode_target) {
                        // we get parent div here not next element, because maybe
                        // we have more than on button, so get textarea direct from parent
                        var $get_parent = $shortcode_button.parent();
                        var $textarea = $get_parent.find('textarea');
                        $textarea.val(base.insertAtChars($textarea, send_to_shortcode)).trigger('change');
                    } else {
                        base.send_to_editor(send_to_shortcode, editor_id);
                    }
                    deploy_atts = null;
                    // this is important here, which fixed duplicated elements
                    $shortcodeload.html('');
                    $d.dialog('close');
                });
                // cloner button
                var cloned = 0;
                $d.on('click', '#shortcode-clone-button', function(e) {
                    e.preventDefault();
                    // clone from cache
    ...
    // ======================================================
    // CSFRAMEWORK ADD METABOXES AJAX
    // ------------------------------------------------------
    $.CSFRAMEWORK.ADD_METABOXES_AJAX = function() {
        ...
        base.init = function() {
             ...
             * because at this time i can't do that, so i will let this code
             * but i will hide it in below in [ JQUERY DOCUMENT READY ]
             * @see the end of this file.
             */

           // show and hide metaboxes options on change custom page template
           ...
           // show and hide metaboxes options on change custom post template
           $('#custom_post_template').on('change', function() {
            
                var post_template = $(this).val();
    ...
    // ======================================================
    // CSFRAMEWORK ADD TAGS
    // ------------------------------------------------------
    $.fn.CSFRAMEWORK_ADD_TAGS = function() {

        return this.each(function( index, element ) {

            // save selectors
            var $this = $(this),
            $tags_input = $this.find('.cs-tags-input'),
            $tags_button = $this.find('.cs-tags-button'),
            $tags_value = $this.find('.cs-tags-value'),
            $tags_list = $this.find('.cs-tags-list ul'),
            $tags_list_div = $this.find('.cs-tags-list'),
            $get_saved_tags_value = $tags_value.val().match( /(?=\S)[^,]+?(?=\s*(,|$))/g ),
            $get_new_tags_value = '';

            $tags_list_div.show();

            // make sure to set empty value if we don't have any tags
            if (!$get_saved_tags_value) {
                $get_saved_tags_value = '';
                $tags_list_div.hide();
            }

            // save remove icon
            var remove_tag_icon = '<span class="cs-tag-remove"><i class="fas fa-times-circle"></i></span> ';

            // show old tags value with remove button
            if ($get_saved_tags_value && $get_saved_tags_value !== '') {
                $get_saved_tags_value.forEach(function(tag) {
                    $tags_list.append('<li>' + remove_tag_icon + tag + '</li>');
                });
            }

            // disaable save button on add new tags
            $tags_input.blur(function() {
                $('.cs-save').prop('disabled', false);
            })
            .focus(function() {
                $('.cs-save').prop('disabled', true);
            ...
            // when click to add new tags
            $tags_button.on('click', function() {

                var $sanitize_input_value = $tags_input.val();
                var rex = /(<([^>]+)>)/ig;
                $sanitize_input_value = $sanitize_input_value.replace(rex , '');
                
                // we have value?
                if ($tags_input && $sanitize_input_value !== '') {
                    
                    // save value
                    $get_new_tags_value = $sanitize_input_value.match( /(?=\S)[^,]+?(?=\s*(,|$))/g );
                    // reset input
                    $tags_input.val('');
                    // insert value to hidden input
                    $get_saved_tags_value = $get_saved_tags_value + ',' + $get_new_tags_value;
                
                    $tags_value.val($get_saved_tags_value);

                    $get_saved_tags_value = $get_saved_tags_value.match( /(?=\S)[^,]+?(?=\s*(,|$))/g );

                    $tags_list_div.show();

                    // add and show new tags
                    $get_new_tags_value.forEach(function(tag) {
                        $tags_list.append('<li>' + remove_tag_icon + tag + '</li>');
                    });
            ...
            // when click to remove tag
            var $removed_tags = '';
            $tags_list.on('click', '.cs-tag-remove', function() {
                
                // get removed tag
                $removed_tags += $(this).parent().text() + ',';

                // remove tag element
                $(this).parent().fadeOut();

                // save removed tags
                var $get_removed_tags = $removed_tags.match( /(?=\S)[^,]+?(?=\s*(,|$))/g );

                // update saved value
                if ($get_removed_tags && $get_removed_tags !== '') {
                    $get_removed_tags.forEach(function(tag) {
                        $get_saved_tags_value = $get_saved_tags_value.filter(function(item) {
                            return item !== tag;
                        });
                    });
                }
    ...
    $.fn.CSFRAMEWORK_STICKYHEADER = function() {
        ...
        if (this.length) {
             ...
             $(window).on('scroll.csStickyHeader', function () {
                 if ($(this).scrollTop() > 1) {
                     $('.cs-header.allow-sticky').addClass("sticky").css('width', $('.cs-body').width());
                 }
                 else {
                     $('.cs-header.allow-sticky').removeClass("sticky").css('width','auto');
                 }
             });
    ...
    // CSFRAMEWORK BACKUP
    // ------------------------------------------------------
    $.fn.CSFRAMEWORK_BACKUP = function() {

        if (this.length) {

            // save selectors
            var $import_button = $(this).find('.cs-import-backup-data'),
                $import_data = $(this).find('.cs-show-import-backup'),
                $export_button = $(this).find('.cs-export-backup-data'),
                $export_data = $(this).find('.cs-show-export-backup'),
                $textarea = $export_data.find('textarea');

            if ($export_data.find('textarea').is(":visible")) {
                $(export_data.find('textarea')).prop('disabled', false);
            }

            // show import data area
            $import_button.on('click', function(e) {
                e.preventDefault();
                $import_data.slideToggle('fast');
            });

            // show export data area
            $export_button.on('click', function(e) {
                e.preventDefault();
                $export_data.slideToggle('fast');
                $export_data.find('textarea').prop('disabled', false);
                if ($textarea.is(':visible')) {
                    $export_data.find('textarea').select();
                    $export_data.find('textarea').prop('disabled', true);
                }
            });
    ...
    // ======================================================
    // CSFRAMEWORK HIDE ADMIN NOTICE
    // ------------------------------------------------------
    $.fn.CSFRAMEWORK_HIDEADMINNOTICE = function() {

        if (this.length) {
            
            // hide admin notice after 2 seconds
    ...
    // CSFRAMEWORK HIDE ADMIN NOTICE
    // ------------------------------------------------------
    $.fn.CSFRAMEWORK_SYSTEM_INFO = function() {

        if (this.length) {
            
            // save selectors
            var $browser_info = $(this).find('#system-info-browser'),
                $expand_info = $(this).find('table thead'),
                $copy_info_button = $('.cs-copy-info'),
                $copied = $('#cs-save-ajax.copy-info'),
                $copy_system_info = $(this).find('.cs-copy-system-info');

            // check device type
            var device_type = 'Computer';
            if (bowser.mobile) { device_type = 'Mobile'; }
            if (bowser.tablet) { device_type = 'Tablet'; }

            // get browser and os info
            if ($browser_info.length) {
                $browser_info.find('.browser-type').text(bowser.name + ' - ' + bowser.version);
                $browser_info.find('.device-type').text(device_type);
                $browser_info.find('.screen-size').text(window.screen.width + ' x ' + window.screen.height);
                $browser_info.find('.viewport-size').text(document.documentElement.clientWidth + ' x ' + document.documentElement.clientHeight);
                $browser_info.find('.operating-system').text(bowser.osname + ' ' + bowser.osversion);
            }

            // show and hide system info
            $expand_info.on('click', function(e) {
                e.preventDefault();

                $(this).parent().toggleClass('opened').toggleClass('closed');
                $(this).find('.cs-expand .fas').toggleClass('fa-chevron-up').toggleClass('fa-chevron-down');
            });

            // copy all system info cs-copy-info
            $copy_info_button.on('click', function(e) {
                e.preventDefault();
                $copy_system_info.text();
                var $tempInput =  $("<textarea>");
                $("body").append($tempInput);
                $tempInput.val($copy_system_info.text()).select();
                document.execCommand("copy");
                $tempInput.remove();
                $copied.hide().fadeIn().delay(500).fadeOut();
            });
    ...
    $(document).ready(function() {
        ...
        $('.cs-field-backup').CSFRAMEWORK_BACKUP();
        $('.cs-field-tags').CSFRAMEWORK_ADD_TAGS();
        $('.cs-system-info-container').CSFRAMEWORK_SYSTEM_INFO();
        $('.cs-settings-error').CSFRAMEWORK_HIDEADMINNOTICE();
        ...
        // $.CSFRAMEWORK.ADD_METABOXES_AJAX(); // just hide for now
