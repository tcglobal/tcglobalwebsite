...
function rangeSliderCss(){
	if(!is_front_page() ){
		...
		wp_enqueue_style( 'style-css', plugins_url('searchtool/css/stylemobile.css'),false,'1.1','all');
	}
}
...
function prefix_add_footer_styles() {
   ...
   // }
    //else{
    	//wp_enqueue_style( 'style-two', get_stylesheet_directory_uri() . '/css/style-two.css');
    	//wp_enqueue_style( 'style-two', get_template_directory_uri() . '/style-two.css');
    //}
    ...
    wp_enqueue_style( 'intlTelInput', get_stylesheet_directory_uri() .'//build/css/intlTelInput.css');
    wp_enqueue_style( 'demo', get_stylesheet_directory_uri() .'/build/css/demo.css');
};
...
if ( $parent->have_posts() ) : 
  while ( $parent->have_posts() ) : $parent->the_post(); 
	...
	$country_img = wp_get_attachment_image_src( get_post_thumbnail_id($pID), 'medium' );
    ...
    $coursecount = get_post_meta( $post->ID, 'course_count', true );
    	...
    	$countryListData .= '<div class="col-sm-12 px-1">
