...
$outputLocationcode = '';
$cityname = '';
$outputLocation = '<div class="ourlocations-list pb-2">
          <div class="container-fluid"><h2 class="mobile-main-heading m-b-50">'.$title.'</h2></div>
			...
			if ( ! empty( $locationCats ) && is_array( $locationCats ) ) {
				...
				foreach ( $locationCats as $locationCat ) 
				{
					...
					foreach ( $sub_categories as $sub_category ){
						...
						$cityname .=	'<div class="filter_link" data-filter="'.str_replace(" ","",$sub_category->name).'" style="cursor: pointer;"><a href="javascript:void(0)" >'.$sub_category->name.'</a></div>';		   
						if($post_query->have_posts()) :
							...
							$locationCourse = get_post_meta( get_the_ID(), 'location_course', true );
								...
								$new_location .= '<div class="location-detail filterDiv '.str_replace(" ","",$sub_category->name).'">
						...
						$subCats .=  '
									<div class="col-sm-6 citynamelist">
...
$outputLocation .= '<div class="row justify-content-center m-t-30 m-b-20">
	<div class="col-sm-5">
		<button type="button" class="btn btn-theme btn-block" id="loadMore">load all locations<img class="w-auto h-auto" src="'.get_template_directory_uri().'/images/tab-downarrow-btn.png" alt=""></button>
	</div>
</div>';
...
$outputLocation .= '</div>
        ...
        <div class="locations-mapplace-center p-t-40">
        <div class="tab-spacing">
        	<div class="col-sm-12"><h2 class="mobile-main-heading">Find Centre location on map</h2></div>
          
        	<div class="col-sm-12 p-t-20 p-b-20">
                <div class="row">
                  <div class="col-7">
                    <p class="locations-switchview-label">Switch to list view</p>
                  </div>
                  <div class="col-5 text-right">
                    <div class="custom-control custom-switch locations-switchview">
          ...
          <div class="find-locations">
		  	...
		  	<div class="sel drop-down hide" id="findlocation">
      ...
      <img src="'.get_template_directory_uri().'/images/location-cursor.png" alt=""  />
		   ...
		   <div id="empty" class="hide no-result m-t-20">Result not found.</div>
		  	<div class="location-vericallist hide" id="myBtnContainer" >'.$new_location.'</div>
			...
			<div class="map-view" id="map_display">
...
$(function () {
    $(".collapse.show .citynamelist").slice(0, 4).show();
    $("#loadMore").on("click", function (e) {
        e.preventDefault();
        $(".collapse.show .citynamelist:hidden").slice(0, 4).slideDown();
        if ($(".collapse.show .citynamelist:hidden").length == 0) {
            $("#load").fadeOut("slow");
        }
        $("html,body").animate({
...
$(".country-list").click(function() {
 ...
 $("#customSwitch1").click(function() {
	...
	function new_location_scroll_fun($atts)
	{
		...
		$display = '<div class="locations-newlocations-list m-b-20">
				      <div class="container-fluid">
				        <h2 class="mobile-main-heading">'.$loc_title.'</h2>
				        <div class="row">
				          <div class="col-sm-12">
				            <h4>'.$loc_sub_title.'</h4>
				           </div>
	...
	// Run a loop and print them all term_id slug
		...
		foreach ( $locationMaps as $locationMap ) 
		{
			...
			$display .= '				
                    <section class="loc_carousel slider locations-newlocations-list px-0 m-b-40">
						'.$displayLocation.'
					</section>
                  </div>
                </div>';
...
while ($single_content_query->have_posts()) : $single_content_query->the_post();
	...
	if($link){
    	...
    	$single_content .= '<div class="locations-experience-centres p-b-30">
