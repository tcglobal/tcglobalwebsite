...
$curPageslug = $post->post_name; // get current page slug
...
if ( ! empty( $terms ) && is_array( $terms ) ) {
    ...
    foreach ( $terms as $term ) {
		...
		if($curPageslug == $term->slug){
			$cssheadingvalue = 'active';
		}
		elseif($curPageslug == 'our-solutions'){
      ...
      $displayCategoryName .= '<a class="nav-link '.$cssheadingvalue.'" id="v-pills-'.$term->slug.'-'.$term->term_id.'-tab" data-toggle="pill" href="#v-pills-'.$term->slug.'-'.$term->term_id.'" role="tab" aria-controls="v-pills-global" aria-selected="false">'.$term->name.'</a>';
	  $j++;	
    }
}
...
if($faq_query->have_posts()) :
	...
	while ($faq_query->have_posts()) : 
			...
			if($k==1)
			{
					...
					$displayFaqContent .= '<div class="tab-pane fade '.$cssvalue.'" id="v-pills-'.$term->slug.'-'.$term->term_id.'" role="tabpanel" aria-labelledby="v-pills-'.$term->slug.'-'.$term->term_id.'-tab"><div class="row justify-content-center"><div class="col-sm-12">'.$searchForm.'</div><div class="accordion" id="accordionExample">';
			}
			...
			if($k==$count){
				...
				$displayFaqContent .= '</div><div class="row"><div class="col-sm-12"><h4><span class="d-block">Didn\'t find the answer?</span>Explore our Help Center</h4> </div><div class="col-sm-12"><a class="text-white" href="'.$helpcenterlink.'" target="_blank"><button type="button" class="btn btn-theme">help center<img src="'.get_template_directory_uri().'/images/whiteforward.png" /></button></a></div></div></div>	</div></div>';
			}
	...
	endwhile; 
		...
		$displayFaqContent .= '<div class="tab-pane fade '.$cssvalue.'" id="v-pills-'.$term->slug.'-'.$term->term_id.'" role="tabpanel" aria-labelledby="id="v-pills-'.$term->slug.'-'.$term->term_id.'-tab""><div class="row justify-content-center"><div class="col-sm-12">
...
$terms = '<div class="tablet-faq-verticaltab">
<div class="bg-color"></div>
<div class="container">
<div class="row">
  <div class="col-4">
    <h2 class="tablet-main-heading">FAQ</h2>
