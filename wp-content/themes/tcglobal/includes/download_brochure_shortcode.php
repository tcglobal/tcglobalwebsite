...
$download .='<div class="desktop-global-learningfeature global-workspace-solutions pt-0 p-b-50">';
$download .='<h2 class="desktop-main-heading">'.$title.'</h2>';
$download .='<div class="container-fluid">';
$download .='<div class="row justify-content-center">';
$download .='<div class="col-sm-9">';
...
if($download_query->have_posts()) :
  while ($download_query->have_posts()) : $download_query->the_post(); 
    ...
    if($formlink == 'schedule_form')
    {
      ...
      $download .='</div>';
	  $download .='</div>';
      $download .='<div class="card-footer"><span class="taglist">Ideal for</span>';
	  ...
	  $download .='<div class="col-sm-6 pl-2">';
      ...
      if($i%2==0){ $download .='</div><div class="card-group">'; }

$i++;
endwhile; 
endif; 
wp_reset_postdata();
$download .='</div></div></div></div></div>';
