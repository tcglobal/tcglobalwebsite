...
$calendly_api = 'https://gei.tcglobal.com/globalapi/calendly_form.php';
...
if($postVal['type'] =='calenldlyScheduleMeeting')
{
	
	$data = [];
	$data['email']		= $postVal['email'];
	$data['mobile'] 	= $postVal['mobile'];
	$apiresponse = curlFunction($calendly_api,$data);	
	$program_output = array('status'=>true,'result'=>json_decode($apiresponse));
	echo json_encode($program_output);
}
