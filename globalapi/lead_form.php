...
/*
$name="RiteshPortal";
$email="RiteshPortal@test.com";
$mobileNumber="2321321321";
//$mobileNumber="7428371531";
$services="Global Learning";
$message="This is test for portal";
...
if($name=='' || $email=='' || $mobileNumber=='' || $services=='' || $message=='' || $ProspectID=='')
{
...
{			
		$query = "INSERT INTO webquery(studid,name,email,phone,city,entereddate,remarks,maincategary,subcategary,type,fname,lname) values($studentID,'$name','$email','$mobileNumber','$cityid',now(),'$message','tcglobalnewsite','','Website','$FirstName','$LastName')";	
			...
			//counselling record
	$query2 = "Insert into CounRec (Date,StudID, InOut,EmpID,Mode,InterviewID,Remarks) values(current_date,$studentID,1,1058,17,null,'Portal ||".$currentPage."||".$FirstName."||".$LastName."||".$email."||".$mobileNumber."||".$services."||".$message."||Website||tcglobalnewsite')";
			$stmt1 = $db_ccpl->prepare($query2);
			$stmt1->execute();	
...
echo $json_encode=json_encode($data);
		...
		if((empty($datereg) && empty($onboarddate)) || $serviceflag=='1')
		{	
	

//echo "nofff studidd";exit;
//echo $studid[0];exit;
