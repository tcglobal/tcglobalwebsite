<?php
	/*
	$firstname		= "RajJourney";;
	$lastname		= "Kumar";
	$mobileNumber	= "9811019534";
	$email			= "rajeevm@thechopras.com";
	$service		= "Global Learning";
	$interested_in	= 'PTE';
	$current_page	= "Global Learning";
	$ipAddress		= "14.141.28.91";
	$choose_date	= "27-Dec-2019";
	$choose_time	= "12:00-13:00 PM";
	$choose_location= "ahmedabad";
...
*/
	...
	else
	{
		...
		*/
		...
		$query3 ="Insert into callback (studid,empid,msg,callbackdate,enterby,entertime,source,webquery)values($studentID,$pcounsel,'".$current_page.'||'.$service.'||'.$choose_date.'||'.$choose_time.'||'.$choose_location."',now(),1487,now(),'Webquery',true)";
		...
		$data_string = '[
				...
				{"Attribute":"mx_Category_Global_Learning_Service", "Value": "'.$interested.'"},		
				...
				{"Attribute":"mx_Branch_visit_schedule", "Value": "'.$eventDate.'"},				
				...
				if($ExceptionMessage == 'A Lead with same Phone Number already exists.')
				{
					...
					if ($err) {
						...
						$email12 = $data_lead[0]->EmailAddress;					
				...
				if($relatedid != '')
				{
					...
					if($recordcount > 0)
					{
						for($i=0;$i<$recordcount;$i++)
						{
							...
							$customdata = 0;
							if($Custom_1==$choose_location and $Custom_2==$activityDate)
							{
								$customdata++;							
							}
						}	
